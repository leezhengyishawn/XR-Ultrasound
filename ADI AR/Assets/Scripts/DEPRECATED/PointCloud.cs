...
/// <summary>
/// This class holds the data of the master point cloud
/// The master point cloud is the 'camera' that shows what the camera is currently seeing.
/// </summary>
public class PointCloud : MonoBehaviour
{
    ...
    [SerializeField]
    PointCloudRenderer m_visualizer;
    ...
    bool isRecordding = false;
    Vector3[] recordedVertices = null;
    Color[] recordedColors = null;
    float pointDistanceIgnore = 0.1f;

    Dictionary<int, Vector3> vertexDict;
    Dictionary<int, Vector3> colorDict;
    ...
    void UpdateRawImage(Texture2D texture, XRCpuImage cpuImage, TextureFormat format)
    {
    ...
    void ReprojectPointCloud()
    {
        ...
        if (vertices == null || colors == null)
        {
            ...
            sizeText.text = String.Format("Width:{0} Height:{1}", width_depth, height_depth);
    ...
    //UI Functions - To be called on external UI
    ...
    public void SwitchMainPointCloudVizualizer(bool flg)
    {
        m_visualizer.gameObject.SetActive(flg);
    }

    public void SwitchLiveCamera(bool flg)
    {
        FindObjectOfType<ARCameraBackground>().enabled = flg;
    }



    public void ChangeScaneFar(System.Single newfar)
    {
        far = newfar;
        farText.text = newfar.ToString();
    }

    public void ToggleRecording(bool recording)
    {
        isRecordding = recording;
        isScanning = !recording;

        recordedVertices = null;
        recordedColors = null;
