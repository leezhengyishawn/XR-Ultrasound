...
public class ConvexHullCalculator : MonoBehaviour
{
	...
	public void CreateConvexHullMesh()
    {
		//transform.parent = m_CameraManager.transform;

		points = new List<Vector3>(GameObject.FindObjectOfType<PointCloud>().GetVertices());

		GenerateHull(points, true, ref verts, ref tris, ref normals);

		Mesh myMesh = new Mesh();
		myMesh.vertices = verts.ToArray();
		myMesh.triangles = tris.ToArray();
		myMesh.normals = normals.ToArray();
		myMesh.colors = GameObject.FindObjectOfType<PointCloud>().GetColors();
