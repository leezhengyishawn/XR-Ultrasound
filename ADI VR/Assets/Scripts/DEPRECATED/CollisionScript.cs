...
public class CollisionScript : MonoBehaviour
{
    ...
    private void Start()
    {
        StartCoroutine(UpdateProbe(0.05f));

        GameObject probeRoot = this.gameObject;
    }

    IEnumerator UpdateProbe(float waitTime)
    {
        while (true)
        {
            yield return new WaitForSeconds(waitTime);
            FindObjectOfType<NetworkManagerUltrasoundRoom>().CallNetworkSetProbePosition();
        }
    }
