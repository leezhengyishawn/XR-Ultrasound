...
{ 
    public class AboutWindow : SingletonEditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.LabelField("Pixyz solutions are edited by Unity Technologies", new GUIStyle(EditorStyles.miniLabel) { alignment = TextAnchor.MiddleCenter });
            GUILayout.EndArea();
        ...
        public static void ShowLicenseInfos(bool boxes = true)
        {
            ...
            if (Pixyz.Core.Native.NativeInterface.IsFloatingLicense()) 
            {
                if (Configuration.CheckLicense())
                {
                    var currentLicenseServer = Pixyz.Core.Native.NativeInterface.GetLicenseServer();
                    EditorGUILayout.BeginVertical("helpBox");
                    EditorGUILayout.LabelField("License Type", "Floating");
                    EditorGUILayout.LabelField("Server Host", currentLicenseServer.serverHost);
                    EditorGUILayout.LabelField("Server Port", currentLicenseServer.serverPort.ToString());
                    EditorGUILayout.EndVertical();
                    hasLicense = true;
                }
                    
            } else {
                var currentNodeLockedLicense = Pixyz.Core.Native.NativeInterface.GetCurrentLicenseInfos();
            ...
            if (Configuration.CheckLicense()) {
                ...
                if (hasLicense && Configuration.CustomVersionTag == "BETA_VERSION" && !Pixyz.Core.Native.NativeInterface.TokenValid("Beta")) {
                    EditorGUILayout.BeginVertical("helpBox");
