...
namespace Pixyz.Plugin4Unity.EditorWindows
{
    ...
    internal static class Menus {
        ...
        {
            ImportSettings settings = AssetDatabase.LoadAssetAtPath<ImportSettings>(Preferences.DefaultImportSettingsLocation + '/' + ImportWindow.DEFAULT_IMPORT_SETTINGS_NAME + ".asset");
            ...
            settings.Verify(ImportSettingsTemplate.Default);
            settings.name = ImportWindow.DEFAULT_IMPORT_SETTINGS_NAME;
            EditorExtensions.SaveAsset(settings, ImportWindow.DEFAULT_IMPORT_SETTINGS_NAME, false, Preferences.DefaultImportSettingsLocation);
        }
        ...
        {
            var importSettings = EditorExtensions.CreateAsset<Pixyz.ImportSDK.ImportSettings>("Import Settings");
        }
        ...
        private static void OpenSupport()
        {
            ...
            if (Application.internetReachability == NetworkReachability.NotReachable) {
                ...
                try {
                    Application.OpenURL("https://unity.com/support-services");
                    success = true;
                } catch {
                    success = false;
                }
