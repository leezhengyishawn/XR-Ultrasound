    ...
    public static class FieldInstanceDrawer
    {
        ...
        public static void DrawGUILayout(this FieldInstance fieldInstance)
        {
            ...
            switch (type.ToString())
            {
                ...
                case "System.String":
                    ...
                    return;
                case "Pixyz.Commons.Utilities.FilePath":
                    FilePath path = (FilePath)value;
                    EditorGUILayout.BeginHorizontal();
                    if (!string.IsNullOrEmpty(name))
                        fieldInstance.value = new FilePath(EditorGUILayout.TextField(new GUIContent(name, tooltip), (FilePath)value), path.IsOutput);
                    else
                        fieldInstance.value = new FilePath(EditorGUILayout.TextField((FilePath)value), path.IsOutput);
                    ...
                    if (GUILayout.Button("...", UnityEditor.EditorStyles.miniButton, GUILayout.Width(25)))
                    {
                        if(path.IsOutput)
                        {
                            fieldInstance.value = new FilePath(EditorUtility.SaveFilePanel("Save as .pxz", Application.dataPath, "exportedModel.pxz", "pxz"), path.IsOutput);
                        }
                        else
                        {
                    ...
                    return;
                case "Pixyz.Commons.Range":
                    //Use GUIUtility.hotControl to detect slider movement
                    fieldInstance.value = (Range)EditorGUILayout.Slider(new GUIContent(name, tooltip), (Range)value, 0f, 100f);
                    return;
                case "System.Object":
                    if (value != null)
                    {
            ...
            else if (type.IsArray)
            {
                ...
                if (!string.IsNullOrEmpty(name))
                    EditorGUILayout.LabelField(new GUIContent(name, tooltip));
                ...
                if (!IsBaseType(elementType) && elementType.IsValueType)
                {
                    ...
                    for (int f = 0; f < ffields.Length; f++)
                    {
                        ...
                        EditorGUI.LabelField(fieldRect, new GUIContent(ffields[f].Name.FancifyCamelCase(), tooltip));
                    }
                    offset += 18;
                }
                /// Draw value setters
            ...
            else if (type.IsClass || type.IsValueType)
            {
                ...
                if(fieldInstance.collapsable)
                {
                    Rect structRect = EditorGUILayout.BeginHorizontal();
                    fieldInstance.unfolded = EditorGUILayout.Foldout(fieldInstance.unfolded, new GUIContent(fieldInstance.name));
                    EditorGUILayout.EndHorizontal();

                    var fields = type.GetFields();
                        
                    if (fieldInstance.unfolded)
                    {
                        for (int f = 0; f < fields.Length; f++)
                        {
                            ...
                            var attributes = fields[f].CustomAttributes;

                            string fieldName = fields[f].Name.FancifyCamelCase();
                            string fieldTooltip = "";

                            bool drawParameter = true;
                            if (attributes.Count() > 0)
                            {
                                if (attributes.First().AttributeType.Equals(typeof(UserParameter)))
                                {
                                    if (attributes.First().ConstructorArguments[3].Value != null && attributes.First().ConstructorArguments[3].Value.ToString().Trim(new Char[] { '"' }) != String.Empty)
                                        fieldName = attributes.First().ConstructorArguments[3].Value.ToString().Trim(new Char[] { '"' }); //Getting name if set

                                    if (attributes.First().ConstructorArguments[2].Value != null && attributes.First().ConstructorArguments[2].Value.ToString().Trim(new Char[] { '"' }) != String.Empty)
                                        fieldTooltip = attributes.First().ConstructorArguments[2].Value.ToString().Trim(new Char[] { '"' }); //Getting tooltip
                                }
                            }
                            if (drawParameter)
                            {
                                EditorGUIUtility.labelWidth = 125;
                                EditorGUILayout.LabelField(new GUIContent(fieldName, fieldTooltip));

                                object fieldValue = DrawGUILayout2(fields[f].GetValue(value), fields[f].FieldType);
        ...
        public static object DrawGUILayout2(this object value, Type type)
        {
            ...
            switch (type.ToString())
            {
                ...
                case "Pixyz.Commons.Utilities.FilePath":
                    EditorGUILayout.BeginHorizontal();
                    FilePath path = (FilePath)value;
                    value = new FilePath(EditorGUILayout.TextField((FilePath)value), path.IsOutput);
                    if (GUILayout.Button("...", UnityEditor.EditorStyles.miniButton, GUILayout.Width(25)))
                    {
                        if(path.IsOutput)
                        {
                            value = new FilePath(EditorUtility.SaveFilePanel("Save as .pxz", Application.dataPath, "exportedModel.pxz", "pxz"), path.IsOutput);
                        }
                        else
                        {
                ...
                case "Pixyz.Commons.Utilities.DynamicEnum":
        ...
        public static object DrawGUI(this object value, Type type, Rect rect)
        {
            ...
            switch (type.ToString())
            {
                ...
                case "Pixyz.Commons.Utilities":
                    FilePath path = (FilePath)value;
                    value = new FilePath(EditorGUI.TextField(new Rect(rect.x, rect.y, rect.width - 35, rect.height), (FilePath)value), path.IsOutput);
