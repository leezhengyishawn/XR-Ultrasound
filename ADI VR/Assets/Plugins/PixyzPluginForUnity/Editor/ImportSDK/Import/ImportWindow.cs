...
namespace Pixyz.ImportSDK {
    ...
    public class ImportWindow : SingletonEditorWindow
    {
        ...
        protected bool _isPostProcessOpen = true;
        ...
        protected void OnGUI()
        {
            ...
            if (compatibility != Compatibility.COMPATIBLE) {
                ...
                if (compatibility == Compatibility.UNTESTED) {
                    message += $"The Pixyz Plugin's compatibility has not been verified yet with Unity {Configuration.UnityVersion} Share with us your feedback if you find any issue";
        ...
        public void OnImportClicked()
        {
            ...
            Core.Native.NativeInterface.SetModuleProperty("IO", "AliasApiDllPath", Preferences.AliasExecutable);
            Core.Native.NativeInterface.SetModuleProperty("IO", "VredExecutablePath", Preferences.VREDExecutable);
            Core.Native.NativeInterface.SetModuleProperty("IO", "RecapSDKPath", Preferences.RecapSDKPath);
        ...
        protected virtual void importEnded(GameObject gameObject)
        {
            ...
            importProgressed(0.85f, "Post-Processing..");
        ...
        {
            GameObject[] rootsBefore = UnityEngine.SceneManagement.SceneManager.GetActiveScene().GetRootGameObjects();

            yield return Dispatcher.DelayFrames(1);
            
            postProcessOver += () => {
                GameObject[] rootsAfter = UnityEngine.SceneManagement.SceneManager.GetActiveScene().GetRootGameObjects();
                // update gameObject in case it has been completely deleted (e.g in Merge By Regions)
                gameObject = rootsAfter[rootsBefore.Length - 1];

                // if post process has generated new objects (e.g in Combine)
                if (rootsBefore.Length < rootsAfter.Length)
                {
                    // put everything under a new root
                    GameObject newRoot = new GameObject(gameObject.name + " (Post-Process)");
                    gameObject.transform.parent = newRoot.transform;

                    for (int i = rootsBefore.Length; i < rootsAfter.Length; i++)
                    {
                        rootsAfter[i].transform.parent = newRoot.transform;
                    }

                    gameObject = newRoot;
                }
                FinalizeProcess(gameObject);

            };

            importProgressed(0.9f, "Post-Processing..");

            RunAdditionalPostProcess();
        }

        protected virtual void FinalizeProcess(GameObject gameObject)
        {
            ImportStamp importedModel = gameObject.GetComponentInChildren<ImportStamp>(true);
            ...
            if (CreatePrefab)
            {
                ...
                try
                {
                    importProgressed(0.95f, "Creating prefab...");
                    ...
                    switch (Preferences.PrefabDependenciesDestination)
                    {
                        case PrefabDependenciesDestination.InPrefab:
                            if (importedModel != null && !AssetDatabase.Contains(importedModel.importSettings))
        ...
        protected virtual void importFailed(Exception e)
        {
